...
mod flash {
    ...
    use traits::{FlashLoanPool, FlashLoanReceiver};
    ...
    impl Flash {
        ...
        pub fn flash_loan(&mut self, target_address: AccountId, pool_address: AccountId, amount: u128, data: Vec<u8>) -> Result<(), FlashLoanError>{
            ...
            let mut underlying_token: contract_ref!(PSP22) = pool.take_cash(amount, target_address).map_err(FlashLoanError::TakeCashFailed)?.into();

            // 2. Call the `on_flash_loan` method of the target
            let mut target: contract_ref!(FlashLoanReceiver) = target_address.into();
            target.on_flash_loan(self.env().caller(), *underlying_token.as_ref(), amount, fee, data).map_err(FlashLoanError::ReceiverFailed)?;
            ...
            underlying_token.transfer_from(target_address, pool_address, new_amount, Vec::new())
                .map_err(FlashLoanError::TransferFailed)?;
        ...
        #[ink(message)]
        pub fn fee_per_million(&self) -> u32 {
        ...
        #[ink(message)]
        pub fn set_fee_per_million(&mut self, fee: u32) -> Result<(), FlashLoanError>  {
            if self.env().caller() != self.fee_admin {
                return Err(FlashLoanError::Unathorized);
            }
